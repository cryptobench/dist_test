...
async def worker(ctx: WorkContext, tasks: AsyncIterable[Task]):
    ...
    async for task in tasks:
        future_result = script.run(
            "/bin/bash", "-c", "chmod 777 /code/run-model.sh && chmod +x /code/run-model.sh")
        future_result = script.run(
            "/bin/bash", "-c", "/usr/bin/python /code/Golem.py")
        script.download_file(
            f"/Golem/output/mse_data.csv", "output/mse_data.csv")
        # script.download_file(
        #     f"/dist_test/output/coefficient_data.csv", "output/coefficient_data.csv")
        yield script
...
async def main():
    package = await vm.repo(
        image_hash="3bd43146e2b614106745a70e04cbffa0462fa66e5dee0c87f4171554",
    )
