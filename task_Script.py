...
async def worker(ctx: WorkContext, tasks: AsyncIterable[Task]):
    ...
    async for task in tasks:
        ...
        script.download_file(f"/code/output/mse_data.csv", "output/mse_data.csv")
        script.download_file(f"/code/output/coefficient_data.csv", "output/coefficient_data.csv")
        yield script
